...
let GitHistoryCommandHandler = class GitHistoryCommandHandler {
    ...
    viewHistory(fileUri, lineNumber) {
        return __awaiter(this, void 0, void 0, function* () {
            const uiService = this.serviceContainer.get(types_2.IUiService);
            const selection = yield uiService.getWorkspaceFolder(fileUri);
            if (!selection) {
                return undefined;
            }
            const workspaceFolder = selection.workspaceFolder;
            const gitRoot = selection.gitRoot;
            const gitService = yield this.serviceContainer.get(types_6.IGitServiceFactory).createGitService(workspaceFolder, gitRoot);
            ...
            const [branchName, startupInfo, locale, gitRootsUnderWorkspace] = yield Promise.all([branchNamePromise, startupInfoPromise, localePromise, gitRootsUnderWorkspacePromise]);
            // Do not include the search string into this
            const fullId = `${startupInfo.port}:${types_6.BranchSelection.Current}:${fileUri ? fileUri.fsPath : ''}:${gitRoot}`;
            const id = md5(fullId); //Date.now().toString();
            yield this.serviceContainer.get(types_5.IWorkspaceQueryStateStore).initialize(id, workspaceFolder, gitRoot, branchName, types_6.BranchSelection.Current, '', fileUri, lineNumber);
            const queryArgs = [
                `id=${id}`, `port=${startupInfo.port}`,
                `file=${fileUri ? encodeURIComponent(fileUri.fsPath) : ''}`,
                `branchSelection=${types_6.BranchSelection.Current}`, `locale=${encodeURIComponent(locale)}`
            ];
