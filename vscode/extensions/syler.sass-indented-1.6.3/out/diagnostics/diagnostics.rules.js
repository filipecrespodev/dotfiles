...
const format_utility_1 = require("../format/format.utility");
...
class DiagnosticRules {
    ...
    check(line) {
        ...
        if (utility_regex_1.isProperty(line.text) && !utility_regex_1.isHtmlTag(line.text)) {
            console.log('PROP', line.text);
            diagnostics.push(...this._CHECK_PROPERTY(line, range));
        }
        else if (utility_regex_1.isVar(line.text)) {
    ...
    _CHECK_PROPERTY(line, range) {
        const diagnostics = [];
        if (utility_regex_1.isProperty(line.text, true)) {
        ...
        if (!format_utility_1.hasPropertyValueSpace(line.text)) {
    ...
    _CHECK_INVALID(line, range) {
        const diagnostics = [];
        if (utility_regex_1.isScssOrCss(line.text)) {
    ...
    _CHECK_VAR(line, range) {
        const diagnostics = [];
        const key = utility_1.splitOnce(line.text, ':').key;
