...
class PlantumlServer {
    ...
    httpWrapper(requestUrl, savePath) {
        return new Promise((resolve, reject) => {
            request({
                ...
                ,
                gzip: true,
                proxy: httpConfig_1.httpConfig.proxy(),
                strictSSL: false
            }, (error, response, body) => {
                let stdout = "";
     ...
     */
    makeURL(diagram, format, index) {
        return [config_1.config.server.replace(/^\/|\/$/g, ""), format, index, this.urlTextFrom(diagram.content)].join("/");
    }
    urlTextFrom(s) {
        let opt = { level: 9 };
