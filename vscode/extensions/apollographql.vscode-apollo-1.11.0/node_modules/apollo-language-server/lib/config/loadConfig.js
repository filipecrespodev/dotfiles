...
async function loadConfig({ configPath, configFileName, requireConfig = false, name, type }) {
    ...
    if (configPath && !loadedConfig) {
        return utilities_1.Debug.error(`A config file failed to load at '${configPath}'. This is likely because this file is empty or malformed. For more information, please refer to: https://bit.ly/2ByILPj`);
    }
    if (loadedConfig && loadedConfig.filepath.endsWith("package.json")) {
        utilities_1.Debug.warning('The "apollo" package.json configuration key will no longer be supported in Apollo v3. Please use the apollo.config.js file for Apollo project configuration. For more information, see: https://bit.ly/2ByILPj');
    }
    if (requireConfig && !loadedConfig) {
        return utilities_1.Debug.error(`No Apollo config found for project. For more information, please refer to:
    ...
    if (fs_1.existsSync(dotEnvPath) && fs_1.lstatSync(dotEnvPath).isFile()) {
        ...
        if (env["ENGINE_API_KEY"]) {
            ...
            nameFromKey = utils_1.getServiceFromKey(env["ENGINE_API_KEY"]);
        }
    }
    let projectType;
    if (type)
        ...
        return utilities_1.Debug.error("Unable to resolve project type. Please add either a client or service config. For more information, please refer to https://bit.ly/2ByILPj");
