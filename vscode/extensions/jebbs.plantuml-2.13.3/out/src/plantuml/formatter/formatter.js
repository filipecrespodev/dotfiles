...
class Formatter {
    ...
    formate(document, options, token) {
        ...
        let analyst = new analyst_1.Analyst(lineTexts, this.rules, !this.config.allowInlineFormat);
        analyst.analysis();
        return this.getEdits(analyst.lines, lines, spaceStr, this.config.allowInlineFormat, this.config.allowSplitLine);
    }
    getEdits(lines, rangeLines, spaceStr, allowInlineFormat, allowSplitLine) {
        let edits = [];
        let blockLevel = 0;
        if (!allowSplitLine) {
            lines.map((line, i) => {
                ...
                if (line.blockElements.length) {
                    switch (line.blockElements[0].type) {
                        case analyst_1.BlockElementType.blockStart:
                            appliedLevel = appliedLevel + 1;
                            delta = -1;
                            break;
                        case analyst_1.BlockElementType.blockEnd:
                            appliedLevel = appliedLevel - 1;
                            delta = 0;
                            break;
                        case analyst_1.BlockElementType.blockAgain:
                            delta = -1;
                        default:
                            break;
                    }
    ...
    formatLine(line) {
        ...
        if (!line.elements.length)
            return "";
        if (!this.config.allowInlineFormat)
            return line.elements.reduce((p, c) => p + c.text, "");
        ...
        for (let i = 0; i < line.elements.length - 1; i++) {
            ...
            switch (thisEl.type) {
                case analyst_1.ElementType.none:
                case analyst_1.ElementType.word:
                    switch (nextEl.type) {
                        case analyst_1.ElementType.none:
                        case analyst_1.ElementType.punctLeftSpace:
                        case analyst_1.ElementType.operater:
                        case analyst_1.ElementType.word:
                ...
                case analyst_1.ElementType.operater:
                    text += " " + getElementText(nextEl);
                    break;
                case analyst_1.ElementType.punctRightSpace:
                    switch (nextEl.type) {
                        case analyst_1.ElementType.none:
                        case analyst_1.ElementType.word:
                        case analyst_1.ElementType.operater:
                    ...
                    break;
                case analyst_1.ElementType.punctLeftSpace:
                case analyst_1.ElementType.connector:
                default:
                    switch (nextEl.type) {
                        case analyst_1.ElementType.operater:
                            text += " " + getElementText(nextEl);
        ...
        function getElementText(el) {
            if (el.type == analyst_1.ElementType.asIs)
                return el.text;
            return el.text.trim();
        }
    }
    splitLine(line, newLineForBlockStart) {
        let splitedLine = null;
        let splitedLines = [];
        let newLineElements = [];
        ...
        for (let b of line.blockElements) {
            if (!isInlineBlock(b, line.blockElements)) {
                for (let e of line.elements) {
                    ...
                    if (e.end <= lastNewLineBlockElementEnd)
                        continue;
                    ...
                    if (e.start >= b.start && e.end <= b.end) {
                        ...
                        if (b.type == analyst_1.BlockElementType.blockStart && !newLineForBlockStart) {
                            pushElement(e, b);
                        }
                        else {
                            if (!inBlockElement)
        ...
        function pushSplitLine() {
            if (splitedLine) {
                splitedLines.push(splitedLine);
                splitedLine = null;
            }
        }
        function pushElement(el, bl) {
            if (!splitedLine)
                splitedLine = { elements: [], blockElements: [] };
            if (el)
                splitedLine.elements.push(el);
            if (bl)
                splitedLine.blockElements.push(bl);
        }
        function isInlineBlock(element, elements) {
            let findBegin = false;
            let findEnd = false;
            return elements.reduce((p, e) => {
                //if already true, or not target block, or is self, return previous value.
                if (p || element.level != e.level || element.index != e.index || element.start == e.start)
                    return p;
                if (element.type == analyst_1.BlockElementType.blockStart || element.type == analyst_1.BlockElementType.blockEnd) {
                    if (e.type == (element.type == analyst_1.BlockElementType.blockStart ? analyst_1.BlockElementType.blockEnd : analyst_1.BlockElementType.blockStart)) {
                        return true;
                    }
                }
                else {
                    if (e.type == analyst_1.BlockElementType.blockStart)
                        findBegin = true;
                    if (e.type == analyst_1.BlockElementType.blockEnd)
                        findEnd = true;
